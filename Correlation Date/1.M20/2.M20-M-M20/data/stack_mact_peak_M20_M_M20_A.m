...
display_start_sec = 0.94 % 计算域时间>展示域
display_end_sec = 0.98
start_sec = 0.955; % 展示域时间
end_sec = 0.975;
...
%     %ｵﾚｶ�\ﾈ�ｸ袒ｵ
    ...
    start4 = 25;
    end4 = 60;
    Time_04=(Time(Ind1) - display_start_sec)*10000;%5000｣ｺfrequency
    np4=round(Time_04); %Index number until the largest peak%;
    
    %search the second peak
    Date4 = Date(np4+start4:np4+end4);
    Time4 = Time(np4+start4:np4+end4);
    SNR4 = SNR_M(np4+start4:np4+end4);
    
    [SNR_k4,num_k4]= findpeaks(SNR4,'minpeakheight', threshold4,'minpeakdistance',min_peak_distance);
    if length(num_k4) == 0
        continue;
    end
    % 排序
    [sort_snr4,sort_num4]=sort(SNR_k4,'descend');
    num_change_k4 = sort_num4(1,:);
    
    Ind4=num_k4(num_change_k4)+ np4 + start4 - 1;
    ...
    start5 = 35;
    end5 = 70;
    Time_05=(Time(Ind1) - display_start_sec)*10000;%5000｣ｺfrequency
    np5=round(Time_05); %Index number until the largest peak%;
    
    %search the second peak
    Date5 = Date(np5+start5:np5+end5);
    Time5 = Time(np5+start5:np5+end5);
    SNR5 = SNR_M(np5+start5:np5+end5);
    
    [SNR_k5,num_k5]= findpeaks(SNR5,'minpeakheight', threshold5,'minpeakdistance',min_peak_distance);
    if length(num_k5) == 0
        continue;
    end
    % 排序
    [sort_snr5,sort_num5]=sort(SNR_k5,'descend');
    num_change_k5 = sort_num5(1,:);
    
    Ind5=num_k5(num_change_k5)+ np5 + start5 - 1;
    ...
    start6 = 45;
    end6 = 80;
    Time_06=(Time(Ind1) - display_start_sec)*10000;%5000｣ｺfrequency
    np6=round(Time_06); %Index number until the largest peak%;
    
    %search the second peak
    Date6 = Date(np6+start6:np6+end6);
    Time6 = Time(np6+start6:np6+end6);
    SNR6 = SNR_M(np6+start6:np6+end6);
    
    [SNR_k6,num_k6]= findpeaks(SNR6,'minpeakheight', threshold6,'minpeakdistance',min_peak_distance);
    if length(num_k6) == 0
        continue;
    end
    % 排序
    [sort_snr6,sort_num6]=sort(SNR_k6,'descend');
    num_change_k6 = sort_num6(1,:);
    
    Ind6=num_k6(num_change_k6)+ np6 + start6 - 1;
    ...
    start7 = 55;
    end7 = 90;
    Time_07=(Time(Ind1) - display_start_sec)*10000;%5000｣ｺfrequency
    np7=round(Time_07); %Index number until the largest peak%;
    
    %search the second peak
    Date7 = Date(np7+start7:np7+end7);
    Time7 = Time(np7+start7:np7+end7);
    SNR7 = SNR_M(np7+start7:np7+end7);
    
    [SNR_k7,num_k7]= findpeaks(SNR7,'minpeakheight', threshold7,'minpeakdistance',min_peak_distance);
    if length(num_k7) == 0
        continue;
    end
    % 排序
    [sort_snr7,sort_num7]=sort(SNR_k7,'descend');
    num_change_k7 = sort_num7(1,:);
    
    Ind7=num_k7(num_change_k7)+ np7 + start7 - 1;
    ...
    start8 = 65;
    end8 = 100;
    Time_08=(Time(Ind1) - display_start_sec)*10000;%5000｣ｺfrequency
    np8=round(Time_08); %Index number until the largest peak%;
    
    %search the second peak
    Date8 = Date(np8+start8:np8+end8);
    Time8 = Time(np8+start8:np8+end8);
    SNR8 = SNR_M(np8+start8:np8+end8);
    
    [SNR_k8,num_k8]= findpeaks(SNR8,'minpeakheight', threshold8,'minpeakdistance',min_peak_distance);
    if length(num_k8) == 0
        continue;
    end
    % 排序
    [sort_snr8,sort_num8]=sort(SNR_k8,'descend');
    num_change_k8 = sort_num8(1,:);
    
    Ind8=num_k8(num_change_k8)+ np8 + start8 - 1;
...
%% save
for inum = 1 : 8%ｸﾝｷ袒ｵﾊ�ﾁｿﾈｷｶｨ
    eval(['dateM20_M_M20_m',num2str(inum),' = Date_m',num2str(inum),';'])
    eval(['snrM20_M_M20_m',num2str(inum),' = snr_m',num2str(inum),';'])
    eval(['timeM20_M_M20_m',num2str(inum),' = Time_m',num2str(inum),';'])
end
Time_M20_M_M20 = Time_p;
Date_M20_M_M20 = Date_p;
snr_M20_M_M20 = snr_x;
save M20_M_M20_A_8peak dateM20_M_M20_m1 dateM20_M_M20_m2 dateM20_M_M20_m3 dateM20_M_M20_m4...
    dateM20_M_M20_m5 dateM20_M_M20_m6 dateM20_M_M20_m7 dateM20_M_M20_m8...
    snrM20_M_M20_m1 snrM20_M_M20_m2 snrM20_M_M20_m3 snrM20_M_M20_m4...
    snrM20_M_M20_m5 snrM20_M_M20_m6 snrM20_M_M20_m7 snrM20_M_M20_m8...
    timeM20_M_M20_m1 timeM20_M_M20_m2 timeM20_M_M20_m3 timeM20_M_M20_m4...
    timeM20_M_M20_m5 timeM20_M_M20_m6 timeM20_M_M20_m7 timeM20_M_M20_m8...
    Time_M20_M_M20 Date_M20_M_M20 snr_M20_M_M20;
