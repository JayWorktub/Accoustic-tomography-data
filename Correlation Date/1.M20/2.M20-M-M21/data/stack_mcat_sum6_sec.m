...
%finish
 ...
 snr_sum=zeros(M,N);
 for ig=4:N-3;
     snr_sum(:,ig)=sum(snr_x(:,ig-3:ig+3),2);
 end
 
  plot3(Time_p,Date_p,snr_sum,'b-','linew',1.8);hold on;
[im_max,ig_max]=max(snr_sum);
 for i=1:N;
     SNR_M=snr_sum(:,i);
    ...
    Time_max=[Time_max;Time_p(Ind(1),i)];
     snr_max=[snr_max;snr_sum(Ind(1),i)];
 end
...
ylabel('Day from June 1, 2017','fontweight','bold','fontsize',18.0);
zlabel('SNR','fontweight','bold','fontsize',18.0);
set(gca,'fontweight','bold','fontsize',18.0);  
save TT3_sum6_max Date_max Time_max snr_max;
